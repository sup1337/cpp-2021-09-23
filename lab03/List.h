...
..., outside the class */
    enum class DeleteFlag {
        LESS, EQUAL, GREATER//ha less re van allitva akkor kitorli az elso olyan elemet ami nala kissebb vagy nagyobb
    };
    void remove(int d, DeleteFlag df = DeleteFlag::EQUAL);//kitorli az elso elemet ami megegyezik a d-vel
/* prints the contents of this list */
    void print() const;
/* forbid copying of lists */
    List(const List&) = delete;//ezt nem kell megcsinalni
    List& operator=(const List&) = delete;//ezt sem
private:
/* number of nodes in the list */
    int nodeCounter = 0;//adattag1
/* a list node */
    struct Node {
    ...
    Node* first; // pointer to the first node//adattag2
};
#endif //LAB02_LIST_H